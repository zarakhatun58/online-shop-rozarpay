...
useEffect(() => {
  ...
  if (currentOrder.status.toLowerCase() !== "paid") return;

  const timers: number[] = [];
  timers.push(
    window.setTimeout(() => 
      setCurrentOrder((prev) => (prev ? { ...prev, status: "shipped" } : prev)), 
    5000)
  );

  timers.push(
    window.setTimeout(() => 
      setCurrentOrder((prev) => (prev ? { ...prev, status: "delivered" } : prev)), 
    5000)
  );
    ...
    <div className="max-w-3xl mx-auto p-6">
      <Card className="p-6 text-center shadow-lg rounded-2xl">
              ...
              <div key={step} className="flex-1 flex flex-col items-center relative">
                {idx < steps.length - 1 && (
                  <motion.div
                    ...
                    className={`absolute top-5 left-1/2 w-full h-1 origin-left ${idx < currentStepIdx ? "bg-green-500" : "bg-gray-300"}`}
                  />
                )}
                <motion.div
                  initial={{ scale: 0 }}
        ...
        <div className="flex flex-wrap gap-3 justify-center">
          <button onClick={() => navigate("/orders")} className="px-5 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
            📦 View All Orders
          </button>
          <Link to={`/track/${currentOrder._id}`} className="px-5 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700">
            🚚 Track This Order
          </Link>
        </div>
      </Card>
